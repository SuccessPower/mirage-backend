    ...
    ILogger<NewsletterDispatchService> logger)
{
    ...
    //
    // The connection retries on transient faults (EnableRetryOnFailure), and Npgsql refuses to let a hand-rolled
    // transaction span those retries — the whole transaction has to be handed to the execution strategy as one
    // retriable unit instead. Opening one directly throws on every pass, which is exactly how due editions went
    // unclaimed and unsent.
    private async Task ClaimDueNewslettersAsync(CancellationToken ct)
    {
        ...
        foreach (var id in dueIds)
        {
            ...
            {
                // A retry replays this delegate after a rollback. Anything the previous attempt left in the change
                // tracker would be written a second time, so every attempt starts from a clean tracker and the
                // delivery rows are built fresh.
                ...
                if (claimed == 0) { await transaction.RollbackAsync(ct); return; }
    ...
    private async Task SendNextBatchAsync(CancellationToken ct)
    {
        ...
        // Same retriable-unit requirement as the claim above. The catch stays *inside* the unit on purpose: this
        // batch has already handed real messages to the provider, and a strategy retry would roll back the sent
        // markers and mail those subscribers a second time. A failed batch is simply left pending for the next
        // tick ten seconds later, where the rows are re-read and nobody is written to twice.
        await db.Database.CreateExecutionStrategy().ExecuteAsync(async () =>
        {
            await using var transaction = await db.Database.BeginTransactionAsync(ct);
            try
